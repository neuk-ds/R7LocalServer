(function (window, undefined) {
	...
	window.Asc.scope = {};
	window.Asc.scope.prototype.clear = function () {
		for (let i in window.Asc.scope) {
	...
	function extend(obj, plugin) {
		...
		let dst = (plugin === undefined) ? {} : plugin;

		for (let prop in obj) {
			if (obj.hasOwnProperty(prop)) {
				dst[prop] = (obj[prop] && typeof obj[prop] === 'object') ? extend(obj[prop]) : obj[prop];
			}
		}
	...
	window.onload = function () {
		...
		xhr.onload = function () {
			...
			if (xhr.status === 200 || (xhr.status === 0 && xhr.readyState === 4)) {
				...
				var obj = { type: 'initialize', guid: window.Asc.plugin.guid };
