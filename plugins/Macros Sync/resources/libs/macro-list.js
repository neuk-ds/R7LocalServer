(function (window) {
    'use strict';
    window.R7SyncLists = function ({ core, $, node, button, run, getData, setUniversal, setExcluded, moveMacro }) {
        ...
        function render() {
            const { doc, library, state, currentSelection, savedSelection } = getData();
            ...
            library.macrosArray.concat(missing).forEach(macro => {
        ...
        function bookPositions() {
            ...
            const rows = Array.from($('listCurrent').children).filter(row => typeof row.animate === 'function');
            return new Map(rows.map(row => [row.getAttribute('data-guid'), row.getBoundingClientRect().top]));
        }
        function animateBookMove(before) {
            if (!before) return;
            for (const row of $('listCurrent').children) {
        ...
        return { isSeparator, realMacros, groups, orderedMacros, selectionBox, render, bookPositions, animateBookMove };
    };
})(window);
